...
export function CreateAnnouncementModal({ isOpen, onClose, onSuccess, classes, initialClassId }: Props) {
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      setFormData(p => ({ 
        ...
        targetClasses: initialClassId ? [initialClassId] : []
      }));
    } else {
      resetForm();
    }
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      data.append('targetType', 'specific_classes');
      ...
      // Use the first class as the primary classId for the route
      const primaryClassId = formData.targetClasses[0];
      await api.post(`/api/teacher/announcements/${primaryClassId}`, data);
          ...
          <form onSubmit={handleSubmit} className="space-y-6">
            ...
            <div className="space-y-4 bg-white/50 p-6 rounded-3xl border border-border/50 shadow-sm">
              <Label className="text-xs font-black uppercase tracking-[0.2em] text-primary flex items-center gap-2">
                <Users className="h-4 w-4" /> Ciblage des classes ({formData.targetClasses.length})
              </Label>
