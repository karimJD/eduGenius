  ...
  return (
    <div className="space-y-8">
      ...
      <PageHeader 
        ...
        description="Consultez votre planning pour ne manquer aucun cours, en direct ou en présentiel."
        icon={CalendarIcon}
        badgeText="Mon Emploi du Temps"
        badgeClassName="bg-indigo-500/10 border-indigo-500/20 text-indigo-400"
        actions={
          <div className="flex items-center gap-4 bg-[#111111] border border-[#222222] p-1.5 rounded-xl">
             <button className="w-10 h-10 rounded-lg flex items-center justify-center text-muted-foreground dark:text-gray-400 hover:text-foreground dark:hover:text-white hover:bg-[#222222] transition-colors">
                <ChevronLeft className="w-5 h-5" />
             </button>
             <span className="font-bold text-foreground dark:text-white px-2">12 Mai - 16 Mai 2025</span>
             <button className="w-10 h-10 rounded-lg flex items-center justify-center text-muted-foreground dark:text-gray-400 hover:text-foreground dark:hover:text-white hover:bg-[#222222] transition-colors">
            ...
            <div className="min-w-[800px]">
               ...
               <div className="grid grid-cols-6 border-b border-[#222222]">
                  <div className="p-4 border-r border-[#222222] bg-[#0a0a0a] flex items-center justify-center">
                     <Clock className="w-5 h-5 text-muted-foreground dark:text-gray-500" />
                  </div>
                  {days.map(day => (
                     <div key={day} className="p-4 border-r border-[#222222] text-center last:border-r-0 bg-[#0a0a0a]">
                        <span className="font-bold text-foreground dark:text-white block">{day}</span>
                     </div>
                  ))}
               </div>
                  ...
                  {hours.map((hour, idx) => (
                     <div key={hour} className="absolute w-full grid grid-cols-6 pointer-events-none" style={{ top: `${idx * 80}px`, height: '80px' }}>
                        <div className="border-r border-b border-[#222222] flex items-start justify-center pt-2">
                           <span className="text-xs font-bold text-muted-foreground dark:text-gray-500">{hour}</span>
                        </div>
                        {days.map((_, i) => (
