  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] space-y-4">
        ...
        <p className="text-gray-400 font-medium">Chargement de votre cours...</p>
      </div>
    );
  }
  ...
  if (error || !course) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] p-6 text-center">
        ...
        <p className="text-gray-400 max-w-md mb-6">{error || "Nous n'avons pas pu trouver ce cours."}</p>
    ...
    <div className="mx-auto space-y-8">
          ...
          <div className="space-y-2">
            <div className="flex items-center gap-3 mb-2">
              <span className="px-3 py-1 bg-blue-500/10 border border-blue-500/20 text-blue-400 text-[10px] font-bold tracking-widest uppercase rounded-full">
                {course.subjectId?.code || 'COURS'}
              </span>
              <span className="text-gray-500 text-xs font-medium">
                {course.chapters?.length || 0} Chapitres publiés
              </span>
            </div>
            ...
            <div className="flex items-center gap-3 text-gray-400 pt-2">
              ...
              <span className="text-sm font-medium">Enseignant: <span className="text-white font-bold">{course.teacherId?.firstName} {course.teacherId?.lastName}</span></span>
            </div>
          </div>

          <div className="bg-[#111111] border border-[#222222] p-5 rounded-3xl min-w-[240px] shadow-xl">
            <div className="flex justify-between items-center mb-3">
              <span className="text-sm font-bold text-gray-400">Progression</span>
              <span className="text-lg font-black text-blue-400">{progress?.overallProgress || 0}%</span>
            </div>
            <div className="h-2.5 w-full bg-[#222222] rounded-full overflow-hidden">
               <motion.div 
                 initial={{ width: 0 }}
      ...
      <div className="pb-20">
        {course.chapters?.length === 0 ? (
          <div className="py-20 text-center border-2 border-dashed border-[#222222] rounded-[3rem] bg-[#0a0a0a]">
            <BookOpen className="w-16 h-16 text-gray-700 mx-auto mb-4 opacity-20" />
            <h3 className="text-xl font-bold text-gray-400">Aucun contenu disponible</h3>
          ...
          <div className="relative border-l-2 border-white/5 ml-4 md:ml-8 space-y-16">
            {course.chapters.map((chapter, cIdx) => (
              ...
              >
                {/* Timeline node */}
                <div className="absolute -left-[17px] md:-left-[25px] top-4 w-8 h-8 md:w-12 md:h-12 rounded-xl md:rounded-2xl bg-gradient-to-br from-blue-500 to-indigo-600 text-white flex items-center justify-center text-sm md:text-xl font-black shadow-xl shadow-blue-500/30 ring-8 ring-[#0a0a0a]">
                  {cIdx + 1}
                </div>

                {/* Chapter Container */}
                <div className="bg-[#111111]/60 backdrop-blur-3xl border border-white/5 rounded-[2.5rem] p-6 md:p-10 space-y-8 hover:border-white/10 transition-colors shadow-2xl relative overflow-hidden">
                  <div className="absolute top-0 right-0 w-64 h-64 bg-blue-500/5 blur-[80px] -mr-32 -mt-32 rounded-full pointer-events-none" />
                  
                  <div className="relative z-10">
                    <h2 className="text-2xl md:text-3xl font-black text-white uppercase tracking-tight leading-tight">{chapter.title}</h2>
                    {chapter.description && (
                      <p className="text-gray-400 font-medium text-sm md:text-base leading-relaxed mt-4 max-w-3xl border-l-2 border-blue-500/40 pl-4">
                        {chapter.description}
                      </p>
                    )}
                  </div>
                  ...
                  <div className="relative z-10 grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    {chapter.materials?.map((material) => (
                      <div 
                        key={material._id}
                        className="group relative bg-[#1a1a1a]/40 backdrop-blur-xl border border-white/5 p-6 rounded-[2rem] hover:border-blue-500/30 hover:bg-white/[0.04] hover:shadow-2xl hover:shadow-blue-500/10 transition-all duration-500 flex flex-col justify-between overflow-hidden"
                      >
                        ...
                        <div className="relative z-10 flex items-start gap-4 mb-4">
                          ...
                          <div className="flex-1 min-w-0">
                            <h4 className="font-bold text-white text-lg truncate group-hover:text-blue-400 transition-colors uppercase leading-tight">
                        ...
                        <div className="relative z-10 flex items-center justify-between pt-4 border-t border-white/5 mt-auto">
                           <span className="text-[10px] font-black text-gray-500 tracking-widest uppercase truncate max-w-[120px]">
                           ...
                           <a 
                             ...
                             className="flex items-center gap-2 px-5 py-2 bg-white/5 border border-white/10 rounded-full text-xs font-bold text-white hover:bg-blue-600 hover:border-blue-500 hover:shadow-lg hover:shadow-blue-500/25 transition-all duration-300"
                           >
                    ...
                    {chapter.exercises?.map((exercise) => (
                      <div 
                        key={exercise._id}
                        className="group relative bg-[#0a0a0a]/60 backdrop-blur-xl border border-blue-500/20 p-6 rounded-[2rem] hover:border-blue-400/50 hover:shadow-2xl hover:shadow-blue-500/10 transition-all duration-500 flex flex-col justify-between overflow-hidden"
                      >
                        ...
                        <div className="relative z-10 flex items-center justify-between pt-4 border-t border-white/5 mt-auto">
                           ...
                           <Link 
                             ...
                             className={cn(
                               "flex items-center gap-2 px-5 py-2 text-white rounded-full text-xs font-bold transition-all duration-300",
                               exercise.hasSubmitted 
                                 ? "bg-white/10 hover:bg-white/20 border border-white/10 shadow-lg" 
                    ...
                    {chapter.materials?.length === 0 && chapter.exercises?.length === 0 && (
                       <div className="col-span-full py-6 text-center text-gray-600 font-bold text-xs uppercase tracking-widest border border-dashed border-[#222222] rounded-3xl">
